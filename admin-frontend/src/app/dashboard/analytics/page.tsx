    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-900 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <div className="text-lg font-medium text-foreground">Loading analytics...</div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-900 relative overflow-hidden">
      ...
      <div className="relative z-10 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-muted-foreground text-lg">Track usage metrics and user interactions</p>
            </div>
            <Link href="/dashboard">
              <Button 
                variant="outline" 
                className="bg-card/80 dark:bg-card/60 backdrop-blur-sm border-border/20 hover:bg-accent/50 text-foreground transition-all duration-200 shadow-sm hover:shadow-md"
              >
                ← Back to Dashboard
              </Button>
            </Link>
        ...
        {/* Environment Filter */}
        <div className="mb-8">
          <div className="bg-card/80 dark:bg-card/60 backdrop-blur-sm rounded-xl p-6 shadow-sm border border-border/20">
            <label className="block text-sm font-medium text-foreground mb-3">Environment Filter</label>
            <select 
              value={selectedEnv} 
            ...
            <div className="mt-4">
              <label className="block text-sm font-medium text-foreground mb-2">Confidence Filter</label>
              <select
                value={lowConfidenceFilter}
                onChange={(e) => setLowConfidenceFilter(e.target.value)}
                className="w-full max-w-xs px-4 py-3 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 bg-background transition-all duration-200"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-card/80 dark:bg-card/60 backdrop-blur-sm border-border/20 shadow-sm hover:shadow-md transition-all duration-200">
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                {analytics?.popular_questions?.map((item, index) => (
                  <div key={index} className="flex justify-between items-start p-4 bg-accent/50 rounded-lg border border-border/50 hover:bg-accent/80 transition-all duration-200">
                    <div className="flex-1">
                      <p className="font-medium text-foreground">{item.question}</p>
                      <p className="text-sm text-muted-foreground mt-1">
                        Confidence: <span className="text-blue-500 font-medium">{(item.avg_confidence * 100).toFixed(1)}%</span>
                      </p>
                    </div>
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                {analytics?.category_distribution?.map((item, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-accent/50 rounded-lg border border-border/50">
                    <span className="capitalize font-medium text-foreground">{item.category}</span>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-muted-foreground font-medium min-w-[2rem]">{item.count}</span>
                    </div>
                  </div>
                )) || <p className="text-muted-foreground text-center py-8">No data available</p>}
              </div>
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                {analytics?.env_distribution?.map((item, index) => (
                  <div key={index} className="flex justify-between items-center p-3 bg-accent/50 rounded-lg border border-border/50">
                    <span className="capitalize font-medium text-foreground">{item.env}</span>
                    <div className="flex items-center gap-3">
                      <div className="w-32 bg-muted rounded-full h-2 overflow-hidden">
                        <div 
        ...
        <div className="mt-12 text-center">
          <div className="bg-card/60 backdrop-blur-sm rounded-xl p-6 shadow-sm border border-border/20 max-w-md mx-auto">
