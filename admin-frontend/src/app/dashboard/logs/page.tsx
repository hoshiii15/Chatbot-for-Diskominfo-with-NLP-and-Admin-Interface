...
export default function ChatLogsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:from-slate-900 dark:via-blue-900 dark:to-indigo-900 p-4 sm:p-6 lg:p-8 relative overflow-hidden">
      {/* Background decorative elements */}
      <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="relative z-10 p-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <Link href="/dashboard">
              <Button
                variant="outline"
                className="bg-card/80 backdrop-blur-sm border hover:bg-accent text-card-foreground hover:text-accent-foreground transition-all duration-200 shadow-sm hover:shadow-md"
              >
                ← Back to Dashboard
              </Button>
            </Link>
          ...
          {/* Filters and Export */}
          <div className="mb-8">
            <div className="bg-card/80 backdrop-blur-sm rounded-xl p-6 shadow-sm border">
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth={2}
                        d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.963-.833-2.732 0L5.268 16.5c-.77.833.192 2.5 1.732 2.5z"
                      />
                    </svg>
                    Manage Logs
                  </Button>
          ...
          {isModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="backdrop-blur-sm bg-card/80 dark:bg-card/60 border-0 shadow-xl rounded-2xl w-[90%] max-w-3xl p-6 max-h-[85vh] overflow-y-auto">
                ...
                <div className="grid gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      value={modalRange}
                      onChange={e =>
                        setModalRange(e.target.value as any)
                      }
                    ...
                    {modalRange === 'pickmonth' && (
                      <input
                        ...
                        className="ml-3 px-3 py-2 border rounded dark:bg-slate-700 dark:border-slate-600"
                      />
                    )}
                    <Button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (modalRange === 'pickmonth' && pickedMonth)
                            params.set('month', pickedMonth)
                          const res = await fetch(
                            `/api/logs?preview=true&${params.toString()}`,
                            {
                              headers: token
                  ...
                  }} className="ml-2 bg-primary text-primary-foreground">Preview</Button>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  }} className="bg-destructive text-destructive-foreground">Delete Selected Range</Button>
                  <Button onClick={async () => {
                  ...
                  }} className="bg-destructive/10 text-destructive border border-destructive/20">Delete All Logs</Button>
                </div>
              </div>
              ...
              <div>
                ...
                {isModalLoading ? (
                  ...
                  <p className="text-sm text-muted-foreground">No preview loaded. Click Preview to fetch logs for the selected range.</p>
                ) : modalLogs.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No logs found for selected range.</p>
                ) : (
                  <div className="space-y-4">
                    {modalLogs.map(m => (
                      <div key={m.id} className="p-3 border rounded bg-muted/50">
                        ...
                        <div className="text-sm text-foreground whitespace-pre-wrap break-words max-h-48 overflow-auto">{sanitizeLogText(m.answer)}</div>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {logs.length === 0 ? (
            <Card className="backdrop-blur-sm bg-card/80 dark:bg-card/60 border-0 shadow-xl">
              <CardContent className="p-12 text-center">
                ...
                <p className="text-muted-foreground text-lg">No chat logs found</p>
                <p className="text-gray-400 text-sm mt-2">Try adjusting your filters or check back later</p>
              </CardContent>
            </Card>
          ) : (
            logs.map((log) => (
              <Card key={log.id} className="backdrop-blur-sm bg-card/80 dark:bg-card/60 border-0 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
                <CardHeader className="border-b border-border/10">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <CardTitle className="text-lg text-foreground mb-3">{log.question}</CardTitle>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {log.category && (
                          <span className="px-3 py-1 bg-muted text-muted-foreground border rounded-full text-xs font-medium">
                            {log.category}
                          </span>
                        )}
                        <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="bg-muted/50 rounded-lg p-4 border">
                        <p className="text-foreground leading-relaxed whitespace-pre-wrap break-words">{sanitizeLogText(log.answer)}</p>
                      </div>
                    </div>
        ...
        {logs.length > 0 && (
          <div className="mt-8 flex items-center justify-center gap-4">
            <Button 
              ...
              className="bg-card/80 backdrop-blur-sm border hover:bg-accent text-card-foreground hover:text-accent-foreground transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <span className="bg-card/80 backdrop-blur-sm px-4 py-2 rounded-lg border text-card-foreground font-medium">
        ...
        <div className="mt-12 text-center">
          <div className="bg-card/60 backdrop-blur-sm rounded-xl p-6 shadow-sm border max-w-md mx-auto">
